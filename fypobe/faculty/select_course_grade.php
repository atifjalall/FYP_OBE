   ...
   <nav class="navbar navbar-default" id="nb">
			<div class="container-fluid" id="nb2">
				...
				if($current_month==1||$current_month==2||$current_month==3||$current_month==4){
					...
					$s='Fall';
					$sql3 ="SELECT b.assigned_course_id,a.title,a.credit,b.semester,b.year,b.offered_course_code,b.sec_no,b.username,a.course_code 
					FROM course a INNER JOIN assigned_course b ON a.course_code = b.offered_course_code WHERE 
					b.username='$username' AND b.semester='$s' AND b.year='$current_year'";
					$flag = 3; 
							}	
				...
				if(($flag==1 && ($result=mysqli_query($connection,$sql1)))||
					...
					if(mysqli_num_rows($result)>0){
		                while($row=mysqli_fetch_assoc($result)){
		                	...
		                	?>
		                	<td><button name="result_submission" class="btn btn btn-danger btn-sm" value="<?php echo $row["assigned_course_id"] ?>">Result Submission</button>
		                	<td><button name="select_grade" class="btn btn btn-danger btn-sm" value="<?php echo $row["assigned_course_id"] ?>">Grade Distribution</button>
							</td>
							<td><button name="select_co_att" class="btn btn btn-danger btn-sm" value="<?php echo $row["assigned_course_id"] ?>">CO Attainment</button>
							</td>
							<td><button name="select_po_att" class="btn btn btn-danger btn-sm" value="<?php echo $row["assigned_course_id"] ?>">PO Attainment</button>
		                	</td>
		                	<?php
		                	echo "</tr>";
		                }
